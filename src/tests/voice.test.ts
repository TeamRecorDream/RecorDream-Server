...
describe("[POST] /voice", () => {
  ...
  it("음성 녹음 업로드 테스트", async () => {
    const res = await req.post("/voice").set("Content-Type", "application/json");
...
describe("[GET] /voice", () => {
  ...
  it("음성 녹음 조회 테스트", async () => {
    const res = await req.get("/voice/62d1745119a54968acdd7270").set("Content-Type", "application/json");
