...
const createRecord = async (req: Request, res: Response) => {
  ...
  if (!err.isEmpty()) {
    res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.RECORD_CREATE_TITLE_FAIL));
  }
  ...
  try {
    ...
    res.status(statusCode.CREATED).send(util.success(statusCode.CREATED, message.RECORD_CREATE_SUCCESS, data));
  } catch (err) {
    console.log(err);
