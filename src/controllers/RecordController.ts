...
const createRecord = async (req: Request, res: Response) => {
  ...
  if (!err.isEmpty()) {
    res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.NO_RECORD_TITLE_FAIL));
  }
...
const updateRecord = async (req: Request, res: Response) => {
  ...
  if (!error.isEmpty()) {
    return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.NO_RECORD_TITLE_FAIL));
  }
